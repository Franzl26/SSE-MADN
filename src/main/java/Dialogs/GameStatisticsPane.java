...
public class GameStatisticsPane extends AnchorPane {
    ...
    public void drawStatistics(GameStatistics stats) {
        ...
        for (int i = 0; i < anzahl; i++) {
            gc.fillText("Platz " + i + ": " + s[i], 5, i * 40 + 30);
        }
        ...
        for (int i = 0; i < anzahl; i++) {
            ...
            for (int j = 1; j < 7; j++) {
                ...
                gc.fillText(j + ": " + (wuerfe[i][j - 1] < 100 ? "  " : "") + wuerfe[i][j - 1] + " = "
                        + percent, baseX, baseY + j * 20 + 10);
                //gc.fillRect(baseX + 135, baseY + j * 20 - 7, percent * 10, 18);
                gc.drawImage(image,baseX + 135, baseY + j * 20 - 7, percent * 10, 18);
            }
        }
        ...
        long time = System.currentTimeMillis() - stats.getStartTime();
        long h = time / 3600;
        long min = (time%3600)/60;
        long sek = (time&60);
        gc.fillText("Spielzeit:    " + (h<10?"0"+h:"") + ":" + (min<10?"0"+min:min) + ":"
                + (sek<10?"0"+sek:sek) + " h", 5, 690);
    }
